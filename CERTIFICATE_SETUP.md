...
Your browser shows **"Your connection isn't private"** on `app.novasafe.io`, `start.novasafe.io`, `internal-docker.novasafe.io` because the **TLS certificate** currently only lists one (or a few) hostnames. Each hostname you serve over HTTPS must be listed in the certificate’s **Subject Alternative Names (SANs)**. If it’s missing, the browser treats it as insecure.

- **Working:** the hostname(s) that were in the cert when it was first issued (e.g. `novasafe.io`).
- **Not working:** any other hostname (e.g. `app.novasafe.io`, `start.novasafe.io`, `internal-docker.novasafe.io`) that was never added to the cert.
...
- https://internal-docker.novasafe.io  
...
| “Your connection isn’t private” on app/start/internal-docker | Those hostnames not in cert | Same: one certbot run with all `-d` names, then reload nginx |
